...
async def msg_lang_setter():
    ...
    if  language == 'chinese':
        ...
        msg_setter.start_command_text = """
This is the official 0x Wallet bot 🤖 .

💰/wallet - Initiates the bot and allows you to add BSC/ETH/SHI addresses💰
...
async def wallet_command_text_fun(user_id):
    ...
    if lang == "chinese":
        return  f"""
💳 您总共有 {no_of_wallet} 个关联钱包 💳
📢 Maestro - 与我们一起做广告@MaestroMarketer"""
    else:
        return f"""
💳 You have a total of {no_of_wallet} linked wallet(s) 💳
...
async def detailwallet_message(wallet_no,wallet_id,wallet_addres,chain_id,pin):
    ...
    try:
        ...
        tot_usd = 0
        tot_eth = 0
        for i,j in enumerate(top_tokens):
            i += 1
            tot_usd = tot_usd + j['tot_amount_in_usd']   
            tot_eth = tot_eth + float(j['total_amount_in_eth'])
            tot_usd = round(tot_usd,3)
            tot_display = f"""
--------------------------------------
📈 Displaying Top {i} Tokens 📉 
💰 Total USD = ${tot_usd} 
💰 Total ETH = {tot_eth} ETH""" 

        return formatted_message , tot_display
    except TypeError:
        return "the wallet has no info"


#for gains 
async def gain_in_wallet(wallet_addres, chain_id,pin):
    if chain_id == 0:
       print("bsc")
    elif chain_id == 1:

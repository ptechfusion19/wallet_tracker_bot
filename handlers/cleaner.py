        ...
        if pair_info:
            ...
            try:
                ...
                if address_checker.lower() in address or quote_token_ads.lower() in address:
                    try:
                        index = address.index(address_checker.lower())
                    except ValueError:
                        try:
                            index = address.index(quote_token_ads.lower())
                        except ValueError:
                            print("no")
                    ...
                    if symbol[index] is not None and address[index] is not None:
                        ...
                        wallet_symbol_and_counthold2.append({'symbol': symbol[index], 'tot_amount_in_usd': total_in_usd , 'total_amount_in_eth':total_in_eth,'price_change':price_change,'market_cap':market_cap , 'address':address[index]})
                        i += 1
                        amount = amount_in_dec[index]
                        symb = symbol[index]
                        try:
                            amount_in_dec.remove(amount)
                            symbol.remove(symb)
                        except ValueError:
                            print("nothing found")
                        try:
